...
class Trainer(BaseTrainer):
    ...
    def move_batch_to_device(batch, device: torch.device):
        ...
        """
        for tensor_for_gpu in ["audio", "audio_class"]:
    ...
    def _log_predictions(
        ...
        audio = audio.cpu()
        pred = pred.cpu()
        audio_class = audio_class.cpu()

        tuples = list(zip(audio, pred, audio_class))
        shuffle(tuples)
        rows = []

        for audio_item, pred_item, audio_class_item in tuples[:examples_to_log]:
            pred_class_id = torch.argmax(pred_item).item()

            row = [
                self.writer.wandb.Audio(audio_item.detach().numpy(), sample_rate=self.sample_rate),
                pred_class_id,
                audio_class_item.item()
            ]
            rows.append(row)
